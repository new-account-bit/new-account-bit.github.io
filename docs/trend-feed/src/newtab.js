...
const MOCK_RSS_ITEMS = {
  "https://feeds.bbci.co.uk/news/world/rss.xml": [
    {
      title: "World markets react to a fresh week of economic data",
      href: "https://example.com/mock/bbc-markets",
      summary:
        "A mock headline used for the web version of Trend Feed, standing in for a real RSS entry when direct browser fetching is blocked.",
      pubDate: "2026-04-09T13:00:00Z"
    },
    {
      title: "Major cities test new transit rules for downtown traffic",
      href: "https://example.com/mock/bbc-transit",
      summary:
        "This sample item shows how the card layout, summary length, and publish date appear on the standalone website.",
      pubDate: "2026-04-08T09:30:00Z"
    }
  ],
  "https://feeds.content.dowjones.io/public/rss/mw_topstories": [
    {
      title: "Stocks open mixed as traders watch inflation signals",
      href: "https://example.com/mock/marketwatch-stocks",
      summary:
        "A placeholder finance story for the public site version, designed to keep the dashboard useful even without extension permissions.",
      pubDate: "2026-04-09T14:15:00Z"
    },
    {
      title: "Energy and rates remain the big themes this week",
      href: "https://example.com/mock/marketwatch-energy",
      summary:
        "Trend Feed can render realistic finance-style content here while you validate the layout and interaction model on GitHub Pages.",
      pubDate: "2026-04-07T16:40:00Z"
    }
  ],
  "https://www.investing.com/rss/news_301.rss": [
    {
      title: "Crypto traders search for direction after a volatile session",
      href: "https://example.com/mock/investing-crypto",
      summary:
        "This mock crypto feed entry demonstrates how category-specific feeds would look on the website version of the product.",
      pubDate: "2026-04-09T11:20:00Z"
    },
    {
      title: "Analysts debate whether momentum is returning to the market",
      href: "https://example.com/mock/investing-analysts",
      summary:
        "Another fallback item to preserve the experience when third-party RSS endpoints reject cross-origin requests from browsers.",
      pubDate: "2026-04-06T07:45:00Z"
    }
  ]
};
...
storageChangeBus.addListener((changes, areaName) => {
...
async function loadDashboard() {
  ...
  if (rssFeeds.length === 0) {
    renderEmptyState(els.rssGrid, {
      ...
      body:
        "Add a few feed URLs to local storage or keep the defaults. On the public website, some feeds may fall back to demo entries because browser RSS requests are often blocked by CORS."
    });
...
async function fetchRssFeed(feed, dateFilter) {
  try {
    ...
    return createMockFeed(feed, dateFilter);
  }
}
...
function createMockFeed(feed, dateFilter) {
  const items = (MOCK_RSS_ITEMS[feed.url] || buildGenericMockItems(feed)).filter((item) =>
    matchesDateFilter(item.pubDate, dateFilter)
  );

  return {
    title: `${feed.title} Demo`,
    url: feed.url,
    items
  };
}

function buildGenericMockItems(feed) {
  return [
    {
      title: `${feed.title} sample headline`,
      href: feed.url,
      summary:
        "This is a mock RSS item rendered for the website version of Trend Feed. In a standalone browser page, many RSS providers block direct client-side access.",
      pubDate: "2026-04-09T10:00:00Z"
    },
    {
      title: `${feed.title} second sample item`,
      href: feed.url,
      summary:
        "The extension can use browser permissions and background logic, but the public website uses placeholder content to demonstrate the interface safely.",
